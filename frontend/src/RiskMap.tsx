...
import { GeoJSON, MapContainer, TileLayer, useMap as useLeafletMap } from "react-leaflet";
...
function LeafletRiskMap({ center, features, buildings }: RiskMapProps) {
  return (
    <MapContainer
...
export default function RiskMap({ center, features, buildings }: RiskMapProps) {
  ...
  if (!apiKey) {
    return <LeafletRiskMap center={center} features={features} buildings={buildings} />;
  }
